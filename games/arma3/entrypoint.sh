...
function RunSteamCMD { #[Input: int server=0 mod=1 optional_mod=2; int id]
    ...
    while (( $updateAttempt < $STEAMCMD_ATTEMPTS )); do # Loop for specified number of attempts
        ...
        if [[ $1 == 0 ]]; then # Server
            ...
            numactl --physcpubind=+0 ${STEAMCMD_DIR}/steamcmd.sh "+login \"${STEAM_USER}\" \"${STEAM_PASS}\"" +workshop_download_item $GAME_ID $2 $validateMod +quit | tee -a "${STEAMCMD_LOG}"
        fi
    ...
    if [[ -n $allMods ]] && [[ ${DISABLE_MOD_UPDATES} != 1 ]]; then
        ...
        do
            if [[ $modID =~ ^[0-9]+$ ]]; then # Only check mods that are in ID-form
                ...
                if [[ ! -d $modDir ]] || [[ ${VALIDATE_MODS} == 1 ]] || [[ ! -f $modDir/timestamp ]] || [[ ( -n $latestUpdate ) && ( $latestUpdate =~ ^[0-9]+$ ) && ( $latestUpdate > $(find $modDir/timestamp | head -1 | xargs stat -c%Y) ) ]]; then
